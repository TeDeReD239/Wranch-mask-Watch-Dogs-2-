...
.bar{height:10px;border:1px solid var(--g);margin:6px 0}
...
function cfg(){fetch('/cmd?ms='+(g('ms').value/100)+'&tl='+g('tl').value+'&th='+g('th').value+'&se='+g('se').value+'&br='+g('br').value)}
let first=true;
setInterval(async()=>{
 ...
 if(first){first=false;g('ms').value=j.ms*100;g('tl').value=j.tl;g('th').value=j.th;g('se').value=j.se;g('br').value=j.br;}
